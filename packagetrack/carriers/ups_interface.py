...
class UPSInterface(BaseInterface):
    ...
    def _send_request(self, tracking_number):
        resp = requests.post(self._api_url, self._build_request(tracking_number), timeout=self.DEFAULT_TIMEOUT).content
        resp = str(resp.decode('ascii', errors='ignore').encode())
        return resp
